...
async def update_seat_amount(id: str, request: UpdateSeatAmountRequest):
    ...
    user_diamond = int(table_balance["diamond"])
    ...
    updated_user_ref = firestore_db.collection("users").document(user_id)    
    # Get the document data
    updated_user_doc = updated_user_ref.get()
    # Check if the document exists
    if not updated_user_doc.exists:
      raise HTTPException(status_code=404, detail="User not found")
    updated_table_balance = updated_user_doc.to_dict()
    # print(updated_table_balance)
    if int(updated_table_balance['diamond'])+amount != int(table_balance["diamond"]):
        return {
            "success":False,
    ...
    if updated_table:
        ...
        transaction_data = {
            ...
            "receiver_type": "game",
            "sender_id": user_id,
            "before_tran_balance": table_balance['diamond'],
